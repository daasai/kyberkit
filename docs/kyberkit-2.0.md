...
| **上下文压缩**         | 6 级压缩 Pipeline（LLM 摘要 + 微压缩 + Session Memory Compact） | 完全缺失                       | 长对话必然 Token 溢出 |
| **记忆提取**          | `extractMemories` + `SessionMemory` 双引擎自动提取           | 仅 `learn()` 手动写入           | 用户无法无感积累知识     |
...
| `LongTermMemory.ts` | **保留** | SQLite 后端可复用 |
